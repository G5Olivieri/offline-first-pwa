      ...
      <div class="bg-white border border-gray-200 rounded-lg shadow-lg p-6">
        ...
        <div class="mb-6">
          <div class="flex items-center justify-between mb-2">
            <span class="text-sm font-medium text-gray-700">{{ currentStepText }}</span>
          ...
          <div class="flex items-center space-x-3">
            <div class="flex-shrink-0">
              <div class="w-5 h-5 rounded-full border-2"
                ...
                <svg v-if="isStepCompleted(SetupStep.LOADING_PRODUCTS)"
                     class="w-3 h-3 text-white" fill="currentColor" viewBox="0 0 20 20">
                  <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/>
                </svg>
                <div v-else-if="isStepActive(SetupStep.LOADING_PRODUCTS)"
                     class="w-2 h-2 bg-blue-600 rounded-full animate-pulse mx-auto mt-0.5"></div>
              </div>
            </div>
            <span class="text-sm text-gray-700">
              Loading products from database
              <span v-if="totalProducts > 0" class="text-gray-500">({{ totalProducts }} products)</span>
            </span>
          </div>
          ...
          <div class="flex items-center space-x-3">
            <div class="flex-shrink-0">
              <div class="w-5 h-5 rounded-full border-2"
                ...
                <svg v-if="isStepCompleted(SetupStep.BUILDING_SEARCH_INDEX)"
                     class="w-3 h-3 text-white" fill="currentColor" viewBox="0 0 20 20">
                  <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/>
                </svg>
                <div v-else-if="isStepActive(SetupStep.BUILDING_SEARCH_INDEX)"
                     class="w-2 h-2 bg-blue-600 rounded-full animate-pulse mx-auto mt-0.5"></div>
              </div>
            </div>
          ...
          <div class="flex items-center space-x-3">
            <div class="flex-shrink-0">
              <div class="w-5 h-5 rounded-full border-2"
                ...
                <svg v-if="isStepCompleted(SetupStep.COMPLETED)"
                     class="w-3 h-3 text-white" fill="currentColor" viewBox="0 0 20 20">
                  <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/>
                </svg>
                <div v-else-if="isStepActive(SetupStep.COMPLETED)"
                     class="w-2 h-2 bg-blue-600 rounded-full animate-pulse mx-auto mt-0.5"></div>
              </div>
            </div>
        ...
        <!-- Error State -->
        <div v-if="hasError" class="mt-6 p-4 bg-red-50 border border-red-200 rounded-md">
...
// Computed properties
const currentStepText = computed(() => setupStore.setupState.currentStepDescription);
...
const errorMessage = computed(() => setupStore.setupState.error || 'An unknown error occurred');
...
const isStepCompleted = (step: SetupStep) => {
  const stepOrder = [SetupStep.LOADING_PRODUCTS, SetupStep.BUILDING_SEARCH_INDEX, SetupStep.COMPLETED];
  const currentIndex = stepOrder.indexOf(setupStore.setupState.currentStep);
  const stepIndex = stepOrder.indexOf(step);
  return currentIndex > stepIndex || (currentIndex === stepIndex && setupStore.setupState.currentStep === SetupStep.COMPLETED);
};
