...
export function useErrorHandler(options: ErrorHandlerOptions = {}) {
  ...
  const errorType = computed(() => {
    ...
    if (state.value.error.message.includes('fetch') || state.value.error.message.includes('network')) {
      return 'network';
    } else if (state.value.error.name === 'ValidationError') {
      return 'validation';
    } else if (state.value.error.message.includes('permission')) {
      return 'permission';
    } else if (state.value.error.message.includes('quota') || state.value.error.message.includes('storage')) {
      return 'system';
    }
    return 'application';
  });
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        if (attempts < maxAttempts) {
          await delay(mergedOptions.retryDelay || 1000 * Math.pow(2, attempts - 1));
        }
      }
    }
  ...
  const shouldRetry = (error: Error): boolean => {
    ...
    if ((error.message.includes('fetch') || error.message.includes('network')) && !onlineStatusStore.isOnline) {
      return false;
    }
  ...
  const getErrorType = (error: Error): 'network' | 'validation' | 'application' | 'system' => {
