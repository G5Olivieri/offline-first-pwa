<template>
  <div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 p-6">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="bg-white/80 backdrop-blur-sm rounded-xl shadow-lg p-6 border border-white/20">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-xs text-gray-500 mt-1">Score: {{ performanceScore }}/100</p>
            </div>
        ...
        <div class="bg-white/80 backdrop-blur-sm rounded-xl shadow-lg p-6 border border-white/20">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-xs text-gray-500 mt-1">{{ formatBytes(memoryUsage.used) }} / {{ formatBytes(memoryUsage.total) }}</p>
            </div>
      ...
      <div class="mb-8">
        <div class="bg-white/80 backdrop-blur-sm rounded-xl shadow-lg p-6 border border-white/20">
          <h3 class="text-lg font-semibold text-gray-900 mb-4">Database Sync Status</h3>
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div v-for="db in databaseStatus" :key="db.name" class="border rounded-lg p-4">
              <div class="flex items-center justify-between mb-2">
                <h4 class="font-medium text-gray-900">{{ db.name }}</h4>
                <div class="flex items-center space-x-2">
                  <div class="w-3 h-3 rounded-full" :class="db.syncStatus.color"></div>
                  <span class="text-sm" :class="db.syncStatus.textColor">{{ db.syncStatus.text }}</span>
                </div>
              </div>
              <div class="space-y-2 text-sm text-gray-600">
                ...
                <div class="flex justify-between">
                  <span>Last Sync:</span>
                  <span>{{ db.lastSync ? db.lastSync.toLocaleTimeString() : 'Never' }}</span>
                </div>
                <div class="flex justify-between">
                  <span>Status:</span>
                  <span>{{ db.status }}</span>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <div class="bg-white/80 backdrop-blur-sm rounded-xl shadow-lg p-6 border border-white/20">
          <h3 class="text-lg font-semibold text-gray-900 mb-4">Performance Timeline</h3>
          <div class="h-64 flex items-end justify-between space-x-2">
            <div v-for="(point, index) in performanceHistory" :key="index" class="flex-1 bg-blue-200 rounded-t-sm relative group">
              <div
        ...
        <!-- Memory Usage Chart -->
        <div class="bg-white/80 backdrop-blur-sm rounded-xl shadow-lg p-6 border border-white/20">
          <h3 class="text-lg font-semibold text-gray-900 mb-4">Memory Usage Trend</h3>
          <div class="h-64 flex items-end justify-between space-x-2">
            <div v-for="(point, index) in memoryHistory" :key="index" class="flex-1 bg-purple-200 rounded-t-sm relative group">
              <div
          ...
          <div class="space-y-4">
            <div v-for="metric in performanceMetrics" :key="metric.label" class="flex items-center justify-between">
              ...
              <div class="flex items-center space-x-2">
                ...
                <div class="w-2 h-2 rounded-full" :class="metric.indicatorColor"></div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white/80 backdrop-blur-sm rounded-xl shadow-lg p-6 border border-white/20">
          ...
          <div class="space-y-4">
            <div v-for="info in systemInfo" :key="info.label" class="flex items-center justify-between">
              ...
              <span class="font-medium text-gray-900 text-right">{{ info.value }}</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="bg-white/80 backdrop-blur-sm rounded-xl shadow-lg p-6 border border-white/20">
          ...
          <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div class="text-center">
              <div class="text-2xl font-bold" :class="networkStatus.color">{{ networkStatus.type }}</div>
              <div class="text-sm text-gray-600">Connection Type</div>
            </div>
            <div class="text-center">
              <div class="text-2xl font-bold text-gray-900">{{ networkStatus.effectiveType || 'Unknown' }}</div>
              <div class="text-sm text-gray-600">Effective Type</div>
            </div>
            <div class="text-center">
              <div class="text-2xl font-bold text-gray-900">{{ networkStatus.downlink || 'Unknown' }}</div>
              <div class="text-sm text-gray-600">Downlink (Mbps)</div>
            </div>
            <div class="text-center">
              <div class="text-2xl font-bold text-gray-900">{{ networkStatus.rtt || 'Unknown' }}ms</div>
      ...
      <div class="bg-white/80 backdrop-blur-sm rounded-xl shadow-lg p-6 border border-white/20">
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-lg font-semibold text-gray-900">Recent Errors & Events</h3>
          <div class="flex items-center space-x-2">
            ...
            <div class="flex items-start justify-between">
              <div class="flex-1">
                <div class="flex items-center space-x-2 mb-1">
                  <span class="text-sm font-medium" :class="getErrorTypeColor(error.type)">{{ error.type }}</span>
                  <span class="text-xs text-gray-600">{{ error.timestamp.toLocaleString() }}</span>
                  <span v-if="error.severity" class="text-xs px-2 py-1 rounded-full" :class="getSeverityBadgeClass(error.severity)">
                    {{ error.severity }}
                  </span>
                </div>
                <p class="text-sm text-gray-700 mb-2">{{ error.message }}</p>
                <details v-if="error.stack" class="text-xs">
                  <summary class="cursor-pointer text-gray-600 hover:text-gray-700">Stack Trace</summary>
                  <pre class="mt-2 text-gray-600 whitespace-pre-wrap bg-gray-50 p-2 rounded text-xs overflow-x-auto">{{ error.stack }}</pre>
                </details>
              </div>
              <button
...
const systemStatus = computed(() => {
  ...
  const hasErrors = errors.value.filter(e => e.severity === 'critical').length > 0
...
const dbStatus = computed(() => {
  ...
  const hasDbErrors = errors.value.some(e => e.message.toLowerCase().includes('database') || e.message.toLowerCase().includes('sync'))
...
const performanceGrade = computed(() => {
  ...
  const gradeConfig = {
    ...
    'Unknown': { color: 'text-gray-600', bgColor: 'bg-gray-100', iconColor: 'text-gray-600' }
  }
...
const performanceMetrics = computed(() => {
  ...
  const getColor = (value: number | undefined, thresholds: { good: number, fair: number }) => {
  ...
  const getIndicatorColor = (value: number | undefined, thresholds: { good: number, fair: number }) => {
  ...
  return [
    ...
    },
    {
      ...
      indicatorColor: getIndicatorColor(m.firstContentfulPaint, { good: 1800, fair: 3000 })
    },
    {
      label: 'Largest Contentful Paint',
      value: formatTime(m.largestContentfulPaint),
      color: getColor(m.largestContentfulPaint, { good: 2500, fair: 4000 }),
      indicatorColor: getIndicatorColor(m.largestContentfulPaint, { good: 2500, fair: 4000 })
    },
    {
      label: 'First Input Delay',
      value: formatTime(m.firstInputDelay),
      color: getColor(m.firstInputDelay, { good: 100, fair: 300 }),
      indicatorColor: getIndicatorColor(m.firstInputDelay, { good: 100, fair: 300 })
    },
    {
      label: 'Cumulative Layout Shift',
      value: m.cumulativeLayoutShift ? m.cumulativeLayoutShift.toFixed(3) : 'N/A',
      color: getColor(m.cumulativeLayoutShift, { good: 0.1, fair: 0.25 }),
      indicatorColor: getIndicatorColor(m.cumulativeLayoutShift, { good: 0.1, fair: 0.25 })
    }
  ]
})
...
const systemInfo = computed(() => [
  ...
  { label: 'Browser', value: systemInfoStore.systemInfo.userAgent?.substring(0, 50) + '...' || 'Unknown' },
  { label: 'Screen', value: `${systemInfoStore.systemInfo.screen.width}x${systemInfoStore.systemInfo.screen.height}` },
  ...
  { label: 'Device', value: systemInfoStore.systemInfo.userAgent?.includes('Mobile') ? 'Mobile' : 'Desktop' }
])
...
const getSyncStatus = (dbName: string) => {
  ...
  const hasSyncErrors = errors.value.some(e =>
...
const clearErrors = () => {
  errors.value = []
  notificationStore.showInfo('Errors Cleared', 'All error logs have been cleared')
}
...
const runDiagnostics = async () => {
  ...
  try {
    ...
    notificationStore.showSuccess('Diagnostics Complete', 'System health check completed successfully')
  } catch (error) {
    addError(error as Error, 'high')
    notificationStore.showError('Diagnostics Failed', 'Failed to complete system diagnostics')
  } finally {
...
const exportLogs = () => {
  ...
  notificationStore.showSuccess('Logs Exported', 'System logs have been exported successfully')
}
...
const clearCache = async () => {
  try {
    ...
    notificationStore.showSuccess('Cache Cleared', 'Browser cache has been cleared')
  } catch (error) {
    addError(error as Error, 'medium')
    notificationStore.showError('Cache Clear Failed', 'Failed to clear browser cache')
  }
}
