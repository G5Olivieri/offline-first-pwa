  ...
  async changeStock(input: Map<string, number>) {
    return this.withErrorHandling(
      async () => {
        ...
        for (const [id, stock] of input.entries()) {
          if (stock < 0) {
            throw new ValidationError(
              `Stock cannot be negative for product ${id}`,
            );
          }
        }
          ...
          ([success, errors], result) => {
            if (result.docs.length === 0) {
              return [
                ...
                [
                  ...errors,
                  new Error(`Product with ID ${result.id} not found`),
                ],
              ];
            }
            ...
            if (Object.getOwnPropertyDescriptor(result.docs[0], "error")) {
              return [
                ...
                [
                  ...errors,
                  new Error(
                    String((result.docs[0] as { error: unknown }).error),
                  ),
                ],
              ];
            }
        ...
        if (errors.length > 0) {
          throw new Error(
            `Errors occurred while fetching products: ${errors.map((e) => e.message).join(", ")}`,
          );
        }
      ...
      async () => {
        ...
        products.forEach((product, index) => {
          try {
            ...
            throw new ValidationError(
              `Validation failed for product at index ${index}: ${(error as Error).message}`,
            );
          }
        });
