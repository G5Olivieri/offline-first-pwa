...
// Global notification utilities for easy access
export const $notify = {
  success: (title: string, message?: string, duration?: number) => {
    const store = useNotificationStore();
    return store.showSuccess(title, message, duration);
  },
  error: (title: string, message?: string, duration?: number) => {
    const store = useNotificationStore();
    return store.showError(title, message, duration);
  },
  warning: (title: string, message?: string, duration?: number) => {
    const store = useNotificationStore();
    return store.showWarning(title, message, duration);
  },
  info: (title: string, message?: string, duration?: number) => {
    const store = useNotificationStore();
    return store.showInfo(title, message, duration);
  },
  clear: () => {
    const store = useNotificationStore();
    return store.clearAllToasts();
  }
};
...
export const $modal = {
  ...
  confirm: (title: string, message: string, options?: Partial<ModalOptions>) => {
    const store = useNotificationStore();
    return store.showConfirm(title, message, options);
  },
  alert: (title: string, message: string, type?: ModalOptions['type']) => {
    const store = useNotificationStore();
    return store.showAlert(title, message, type);
  }
};
